...
WHERE `joindate`=
(SELECT MAX(`joindate`) FROM `Members` AS max_date)
...
SELECT DISTINCT CONCAT(`firstname`,' ',`surname`) AS full_name FROM `Members` 
WHERE memid IN
(SELECT distinct memid FROM Bookings WHERE facid IN
...
WHERE  ((m.memid = 0 AND b.slots * f.guestcost > 30) OR
      ...
      AND CONVERT(DATE,b.starttime)='2012-09-14'
ORDER BY cost DESC


/* Q9: This time, produce the same result as in Q8, but using a subquery. */
SELECT f.name AS facility, CONCAT(m.firstname,' ',m.surname) AS full_name, b.slots, f.membercost, f.guestcost, b.starttime,
...
WHERE b.starttime IN 
    (SELECT `starttime` FROM `Bookings` WHERE `starttime`>='2012-09-14' AND `starttime`<'2012-09-15')
AND
